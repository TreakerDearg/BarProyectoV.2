...
export const getDashboardStats = async (req, res, next) => {
  try {
    ...
    // Common Data
    const [orders, tables, inventoryCount, reservationsToday, lowStockCount, outOfStockCount] = await Promise.all([
    ...
    const stats = {
      kpis: calculateKPIs(orders, reservationsToday, today),
    ...
    if (view === "all" || view === "service") {
      stats.service = {
        ...
        recentReservations: await Reservation.find({ startTime: { $gte: today } }).limit(5).sort({ startTime: 1 }).lean(),
      };
    ...
    logger.error(`[Dashboard] Critical Error: ${error.message} \nStack: ${error.stack}`);
    return next(error);
  }
};
...
const calculateKPIs = (orders, reservationsToday, today) => {
  ...
  return { totalSales, totalOrders, todayOrders, avgTicket, reservationsToday };
};
...
const calculateOperationalLoad = (orders, type) => {
  const pendingItems = orders.flatMap(o => o.items)
    .filter(i => i.type === type && (i.status === "pending" || i.status === "preparing"));
  return Math.min(pendingItems.length * 10, 100);
};
