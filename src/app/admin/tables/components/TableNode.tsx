...
const statusConfig: any = {
  available: { label: "Libre", accentBg: "bg-gold", accentRing: "ring-gold", glow: "shadow-gold-glow", bg: "bg-gold/10", border: "border-gold/30", text: "text-gold" },
  occupied: { label: "Ocupada", accentBg: "bg-orange", accentRing: "ring-orange", glow: "shadow-orange-glow/50", bg: "bg-orange/10", border: "border-orange/30", text: "text-orange" },
  reserved: { label: "Reservada", accentBg: "bg-blue", accentRing: "ring-blue", glow: "shadow-blue-glow/50", bg: "bg-blue/10", border: "border-blue/30", text: "text-blue" },
  maintenance: { label: "Fuera", accentBg: "bg-red", accentRing: "ring-red", glow: "shadow-red-glow/50", bg: "bg-red/10", border: "border-red/30", text: "text-red" },
};
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${selected ? `ring-2 ${config.accentRing} ring-offset-4 ring-offset-bg z-10 ${config.glow}` : "z-0"}
      `}
    >
      {/* GLOW DECOR */}
      <div className={`absolute -top-1/2 -right-1/2 w-full h-full rounded-full opacity-[0.08] blur-3xl ${config.accentBg}`} />
      
      {/* STATUS INDICATOR */}
      <div className="absolute top-4 right-4">
        <div className={`w-2 h-2 rounded-full ${config.accentBg} ${table.status === 'occupied' ? 'animate-pulse' : ''} shadow-lg`} />
      </div>
      ...
      {/* SELECTION INDICATOR */}
      {selected && (
        <div className={`absolute inset-x-0 bottom-0 h-1 ${config.accentBg}`} />
      )}
