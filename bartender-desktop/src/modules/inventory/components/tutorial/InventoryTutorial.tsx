...
import { inventoryTutorialSteps } from "./inventoryTutorialSteps";
...
export default function InventoryTutorial({
  isOpen,
  onClose,
  onComplete,
}: Props) {
  ...
  const step = inventoryTutorialSteps[currentStep];
  const Icon = step.icon;
  const isLast = currentStep === inventoryTutorialSteps.length - 1;
      ...
      <div className="relative w-full max-w-3xl nebula-panel overflow-hidden shadow-2xl border-violet-400/20">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-ivory">
                Tutorial · Bóveda de Insumos
              </h2>
              <p className="text-xs text-muted mt-0.5">
                Guía para usar el sistema de inventario
              </p>
            </div>
          </div>
          <button
        ...
        <div className="h-1 bg-surface-3">
          <div
            ...
            style={{
              width: `${((currentStep + 1) / inventoryTutorialSteps.length) * 100}%`,
            }}
          />
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-violet-300/90 px-2 py-0.5 rounded-full bg-violet-500/15 border border-violet-400/20">
                Paso {currentStep + 1} de {inventoryTutorialSteps.length}
              </span>
        ...
        <div className="flex items-center justify-between p-6 border-t border-white/8 bg-surface-3/30 gap-4">
          ...
          <div className="flex items-center gap-1.5">
            {inventoryTutorialSteps.map((_, i) => (
              <button
                key={i}
                type="button"
