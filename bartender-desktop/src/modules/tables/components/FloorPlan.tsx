...
export default function FloorPlan({
  ...
  const handleContainerDrop = (e: React.DragEvent) => {
    ...
    const clampedX = Math.round(Math.max(8, Math.min(92, newX)));
    const clampedY = Math.round(Math.max(10, Math.min(90, newY)));

    onTableLayoutChange(tableId, clampedX, clampedY);
  };
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-muted">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <p className="text-[8px] md:text-[9px] text-dim font-bold uppercase tracking-[0.2em]">
                {viewType === "spatial" ? "VISTA ESPACIAL EN TIEMPO REAL" : "VISTA CUADRÍCULA OPERATIVA"}
              </p>
            </div>
          </div>
        </div>

        {/* Botón rápido modo editor (solo en vista espacial) */}
        {viewType === "spatial" && onToggleEditMode && (
          <button
            type="button"
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all border ${
              isEditMode
                ? "bg-gold text-neutral-950 border-gold shadow-lg shadow-gold/20"
                : "bg-white/5 text-muted hover:text-ivory hover:bg-white/10 border-white/10"
            }`}
          >
            <span className={`w-2 h-2 rounded-full ${isEditMode ? "bg-black animate-ping" : "bg-gold"}`} />
            <span>{isEditMode ? "Guardar distribución" : "Editar plano"}</span>
          </button>
        )}
      </div>
      ...
      <div className="relative flex-1 bg-black/40 rounded-[2rem] md:rounded-[2.5rem] border border-white/5 overflow-hidden group/floor shadow-2xl min-h-0">
        ...
        {viewType === "spatial" ? (
          ...
          >
            ...
            {isEditMode && (
              <div className="absolute top-4 left-4 z-20 flex items-center gap-2 bg-gold/20 border border-gold/40 rounded-xl px-3.5 py-2 backdrop-blur-md shadow-lg shadow-black/80 animate-pulse">
                <span className="w-2 h-2 rounded-full bg-gold animate-ping" />
                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gold">
                  Modo Editor: Arrastra las mesas para organizar el salón
                </span>
              </div>
            )}
            ...
            {tables.length === 0 && !loading && (
              <div className="h-full flex flex-col items-center justify-center p-6 text-center">
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10 text-gold mb-3">
                  <LayoutGrid size={36} />
                </div>
                <h3 className="text-sm font-bold text-ivory uppercase tracking-wider mb-1">
                  No hay mesas en esta zona
                </h3>
                <p className="text-xs text-muted max-w-sm mb-4">
                  Cambia de filtro o crea una nueva mesa para comenzar a operar esta área.
                </p>
                {onCreateTable && (
                  <button
                    type="button"
                    onClick={onCreateTable}
                    className="px-4 py-2 rounded-xl bg-gold/20 border border-gold/40 text-gold text-xs font-bold uppercase tracking-wider hover:bg-gold/30 transition-all"
                  >
                    + Crear mesa en esta zona
                  </button>
                )}
              </div>
            )}
        ...
        <div className="absolute bottom-4 right-4 flex flex-wrap items-center justify-center gap-2 md:gap-3.5 bg-black/90 backdrop-blur-2xl px-3 md:px-5 py-2.5 rounded-xl md:rounded-2xl border border-white/10 shadow-2xl max-w-[95%] z-20 pointer-events-none">
