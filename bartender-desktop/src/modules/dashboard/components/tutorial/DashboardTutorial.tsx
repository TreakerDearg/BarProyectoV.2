...
import { dashboardTutorialSteps } from "./dashboardTutorialSteps";
...
export default function DashboardTutorial({
  ...
  const step = dashboardTutorialSteps[currentStep];
  const Icon = step.icon;
  const isLast = currentStep === dashboardTutorialSteps.length - 1;
      ...
      <div className="relative w-full max-w-3xl nebula-panel overflow-hidden shadow-2xl border-violet-400/20">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-ivory">
                Tutorial · Panel Nebula
              </h2>
              <p className="text-xs text-muted mt-0.5">
                Guía para usar el centro de operación
              </p>
            </div>
          </div>
          <button
        ...
        <div className="h-1 bg-surface-3">
          <div
            ...
            style={{
              width: `${((currentStep + 1) / dashboardTutorialSteps.length) * 100}%`,
            }}
          />
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-violet-300/90 px-2 py-0.5 rounded-full bg-violet-500/15 border border-violet-400/20">
                Paso {currentStep + 1} de {dashboardTutorialSteps.length}
              </span>
        ...
        <div className="flex items-center justify-between p-6 border-t border-white/8 bg-surface-3/30 gap-4">
          ...
          <div className="flex items-center gap-1.5">
            {dashboardTutorialSteps.map((_, i) => (
              <button
                key={i}
                type="button"
